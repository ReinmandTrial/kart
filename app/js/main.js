jQuery(document).ready(function ($) {
   ...
   if ($(window).width() >= 768) {
      $($('body').find('.product__categories-block')).each(function () {
         ...
         while (
            $(this).width() >= $(this).closest('.product__categories').width()
         ) {
            hiddenWrap.show();
   ...
   $(document).on('click', function (e) {
      if (
         !$(e.target).closest('.js-notification-body').length &&
         !$(e.target).closest('.js-notification').length
      ) {
   ...
   const sliderInnovationTabs = new Swiper(
      '.slider-innovation-tabs__pagination',
      {
         slidesPerView: 'auto',
   ...
   const sliderInnovationImages = new Swiper(
      '.innovation-categories-images-slider__container',
      {
         slidesPerView: 1,
         watchSlidesProgress: true,
         simulateTouch: false,
   ...
   const sliderInnovationMain = new Swiper(
      '.slider-innovation-tabs__container',
      {
         navigation: {
   ...
   $($('.woocommerce #reviews #comments ol.commentlist').find('li')).each(
      function () {
   ...
   if ($('body').hasClass('post-type-archive-product')) {
      ...
      $('.wpfMainWrapper').append(
         '<button class="icon icon-close wpfFilterContent__close"></button>'
      );

      $('.wpfMainWrapper').wrap(
         "<div class='category__filters'><div class='category__filter-wrap'></div></div>"
      );
      $('.woocommerce-ordering').appendTo('.category__filters');

      $('.woocommerce-ordering').wrap(
         "<div class='category__sort-wrap'></div>"
      );
   ...
   $(
      '.product__social-share .ya-share2 .ya-share2__item_service_facebook a'
   ).append('<span class="socials-link icon icon-facebook"></span>');

   $(
      '.product__social-share .ya-share2 .ya-share2__item_service_whatsapp a'
   ).append('<span class="socials-link icon icon-call"></span>');
   $(
      '.product__social-share .ya-share2 .ya-share2__item_service_twitter a'
   ).append('<span class="socials-link icon icon-twitter"></span>');
   $(
      '.product__social-share .ya-share2 .ya-share2__item_service_telegram a'
   ).append('<span class="socials-link icon icon-telegram"></span>');

   // prodact page end

   // acticle
   $(
      '.post__to-share-socials .ya-share2 .ya-share2__item_service_facebook a'
   ).append('<span class="socials-link icon icon-facebook"></span>');
   $(
      '.post__to-share-socials .ya-share2 .ya-share2__item_service_whatsapp a'
   ).append('<span class="socials-link icon icon-call"></span>');
   $(
      '.post__to-share-socials .ya-share2 .ya-share2__item_service_pinterest a'
   ).append('<span class="socials-link icon icon-pint"></span>');
   // acticle end

   // news
   $(
      '.news-sidebar__socials .ya-share2 .ya-share2__item_service_facebook a'
   ).append('<span class="socials-link icon icon-facebook"></span>');
   $(
      '.news-sidebar__socials .ya-share2 .ya-share2__item_service_twitter a'
   ).append('<span class="socials-link icon icon-twitter"></span>');
   $(
      '.news-sidebar__socials .ya-share2 .ya-share2__item_service_pinterest a'
   ).append('<span class="socials-link icon icon-pint"></span>');
   // news end
   ...
   $('.js-tabs-head-item').on('click', function () {
      ...
      if (!$(this).hasClass('active')) {
         ...
         if (custom_target_group && current_link_target) {
            $(
               `[data-custom-target-group="${custom_target_group}"]`
            ).removeClass('active');
            $(
               `[data-custom-target-group="${custom_target_group}"][data-custom-target="${current_link_target}"]`
            ).addClass('active');
            $(window).resize();
         }
   ...
   $('.tabs__item[data-tab-open="reviews"] span').text(
      '(' + amountComments + ')'
   );
   ...
   $('.log-reg .input, .log-reg .input__text').on(
      'focusin click input',
      function () {
         $(this).closest('.input__box').addClass('fill');
      }
   ...
   $('#register_btn').on('click', function (event) {
      ...
      if (check_first_name == false) {
         let register_first_name_inp = document.getElementById(
            'register_first_name'
         ).nextElementSibling;
         register_first_name_inp.innerText = 'השדות צריכים להכיל אותיות בלבד';
      } else if (check_last_name == false) {
         let register_last_name_inp =
            document.getElementById('register_last_name').nextElementSibling;
   ...
   function miniCartAjaxUpdate() {
      $.ajax({
         ...
         success: function (response) {
            ...
            let miniCartItemsContainer =
               document.getElementById('sidebar-all-items');
   ...
   $('.add_to_cart_with_quantity_variable_product_shop_page').on(
      ...
      function (e) {
         ...
         let thisBtn = this;
         let quantity =
            $this.get(0).nextElementSibling.nextElementSibling
               .nextElementSibling.value;
         let variation_id = $this.get(0).nextElementSibling.value;
         let productID =
            $this.get(0).nextElementSibling.nextElementSibling.value;
         ...
         if (parseInt(quantity) == 0) {
            alert(
               'warning first add_to_cart_with_quantity_variable_product_shop_page'
            );
         } else {
            $.ajax({
               type: 'post',
               ...
               success: function (response) {
                  $.ajax({
                     ...
                     success: function (response_s) {
                        if (response_s.stock_status == true) {
                           addToCartWithQuantityVariableProduct(
                              variable_product_obj
                           );
                        } else {
                           alert(
                              'warning add_to_cart_with_quantity_variable_product_shop_page'
                           );
                        }
                     },
                  });
   ...
   $('.add-to-cart-with-quantity-variable_product-btn').on(
      ...
      function (e) {
         ...
         if (variation_id) {
            ...
            if (parseInt(quantity) == 0) {
               alert(
                  'warning add-to-cart-with-quantity-variable_product-btn 1'
               );
            } else {
               $.ajax({
                  type: 'post',
                     ...
                     $.ajax({
                        ...
                        success: function (response_s) {
                           if (response_s.stock_status == true) {
                              ...
                              alert(
                                 'not work addToCartWithQuantityVariableProduct 2  '
                              );
                           }
                        },
                     });
            ...
            alert(
               'not work warning add-to-cart-with-quantity-variable_product-btn '
            );
         }
      }
   );
   ...
   const langFlagsHeader = $('.header__topline-lang #flags')
      .clone()
      .addClass('clone');
   ...
   const langFlagsMob = $('.burger-mobile__lang #flags')
      .clone()
      .addClass('clone');
   ...
   $(document).on('click', function (e) {
      if (
         !e.target.closest('.popup-exit__content') &&
         !e.target.closest('.header__profile-exit')
      ) {
         $('.popup-exit').fadeOut();
      }
   });
   ...
   // tags
   if ($(window).width() >= 576) {
      const newsTags = $('.news-sidebar__tags-wrapper .tags').find(
         '.news-sidebar__tag'
      );

      if (newsTags.length > 30) {
         $('.news-sidebar__tags-wrapper')
            .find('.product__categories-more')
            .show();
   ...
   function setPos() {
      ...
      document.querySelector('.dropdown-pos-to').style.right =
         posFromRight - rightFromContainer + 'px';
   }
   setPos();
   // position for header dropdown
   ...
   $('.saved-tabs-edit__input, .saved-tabs__remove-tab').on(
      'click',
      function (e) {
         e.stopPropagation();
      }
